...
namespace angel {
	...
	pLodData LodFile::LoadFileData( const std::string& fname) {
		pak_item_t pakitm = FindFile(fname);
		if(pakitm.size < 0 )
			return pLodData();
	...
	pLodData LodFile::LoadFileHdr( const std::string& fname) {
		pak_item_t pakitm = FindFile(fname);
		if(pakitm.size < 0  )
			return pLodData();
		if(paks[pakitm.lodindex].dirname == "maps")
	...
	pLodData LodFile::LoadFileHdrName( const std::string& fname) {
		pak_item_t pakitm = FindFile(fname);
		if(pakitm.size < 0 )
			return pLodData();
		if(paks[pakitm.lodindex].dirname == "maps")
